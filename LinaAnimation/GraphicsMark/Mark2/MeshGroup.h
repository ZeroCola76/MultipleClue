...
public:
	virtual void Initialize(ComPtr<ID3D11Device>& device, vector<NestGraphics::MeshData>* modelMeshDataes);	// assimp로  정보를 가져오는 메쉬 초기화
	//virtual void Initialize(ComPtr<ID3D11Device>& device, std::vector<NestGraphics::MeshData> meshes);		// 큐브 매핑시 쓰이는 초기화
