...
void MeshGroup::Initialize(ComPtr<ID3D11Device>& device, const std::wstring& filename)
{
...
void MeshGroup::Initialize(ComPtr<ID3D11Device>& device)
{
	...
	for (const auto& meshData : GetMeshType())
	{
		auto newMesh = std::make_shared<NestGraphics::Mesh>();
	...
	m_pNormalLines = std::make_shared<NestGraphics::Mesh>();

	std::vector<NestGraphics::Vertex> normalVertices;
...
void MeshGroup::Update(Vector3 vector, Matrix view, Matrix proj)
{
	///얘는 추상함수가 되어야하는게 맞는데...중간 클래스를 만들어야할까?
	///
}

std::vector<NestGraphics::MeshData> MeshGroup::GetMeshType()
{
	return std::vector<NestGraphics::MeshData>();
}
